...
-f h5seurat     \
-o ./2-Diffexp/After-vs-Before     \
--assay RNA     \
--dataslot data,counts     \
-j 10  \
--predicate "clusters %in% c('2')" \
diffexp     \
...
-g ./2-Diffexp/After-vs-Before/sampleid_After-vs-Before-all_diffexp_genes.xls \
--anno /data/database/cellranger-refdata/refdata-gex-GRCh38-2020-A/annotation//gene_annotation.xls

Rscript   /public/scRNA_works/pipeline/oesinglecell3/exec/sctool  annotation \
-g ./2-Diffexp/After-vs-Before/sampleid_After-vs-Before-diff-pval-0.05-FC-1.2.xls \
--anno /data/database/cellranger-refdata/refdata-gex-GRCh38-2020-A/annotation//gene_annotation.xls
...
--predicate "clusters %in% c('2') & sampleid %in% c('After','Before')" \
diff_heatmap \
-d ./2-Diffexp/After-vs-Before/sampleid_After-vs-Before-diff-pval-0.05-FC-1.2.xls \
-n 20 \
-g sampleid \
--group_colors customecol2 \
--sample_ratio 0.8

rm ./2-Diffexp/After-vs-Before/sampleid_After-vs-Before-all_diffexp_genes.xls ./2-Diffexp/After-vs-Before/sampleid_After-vs-Before-diff-pval-0.05-FC-1.2.xls

perl /gpfs/oe-scrna/ziqingzhen/script/enrichment/enrich_go_kegg.pl -infile 2-Diffexp/After-vs-Before/*-vs-*-diff-*.xls \
...
-outdir 2-Diffexp/After-vs-Before/enrichment  \
-shelldir 2-Diffexp/After-vs-Before/enrichment_sh
